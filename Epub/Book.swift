...
public class Book: NSObject {
    ...
    public func fetchFromZip(_ fileName: String) -> Data? {
        ...
        let fileSize = zip?.getCurrentFileInZipInfo().length
        let fileData = NSMutableData(capacity: Int(fileSize!))
        fileStream?.readData(withBuffer: fileData!)
    ...
    private func parseContainerData(_ data: Data) {
        ...
        let container = root.firstChild(tag: XML_ELEMENT_CONTAINER, inNamespace: XML_NAMESPACE_CONTAINER)
        let rootFiles = container?.firstChild(tag: XML_ELEMENT_ROOTFILES)
    ...
    private func parseOpfFile(_ data: Data) {
        ...
        let opf = root.firstChild(tag: XML_ELEMENT_PACKAGE, inNamespace: XML_NAMESPACE_PACKAGE)
        
        let meta = opf?.firstChild(tag: XML_ELEMENT_METADATA)
        parseMetadata(meta)
        
        let manifest = opf?.firstChild(tag: XML_ELEMENT_MANIFEST)
        parseManifest(manifest)
        
        let spineElm = opf?.firstChild(tag: XML_ELEMENT_SPINE)
        parseSpine(spineElement: spineElm)
    }
    
    private func parseMetadata(_ meta: XMLElement?) {
        metadata.title = meta?.firstChild(tag: XML_ELEMENT_TITLE, inNamespace: XML_METADATA_NAMESPACE)?.stringValue
        metadata.creator = meta?.firstChild(tag: XML_ELEMENT_CREATOR, inNamespace: XML_METADATA_NAMESPACE)?.stringValue
        metadata.publisher = meta?.firstChild(tag: XML_ELEMENT_PUBLISHER, inNamespace: XML_METADATA_NAMESPACE)?.stringValue
        metadata.language = meta?.firstChild(tag: XML_ELEMENT_LANGUAGE, inNamespace: XML_METADATA_NAMESPACE)?.stringValue
        metadata.identifier = meta?.firstChild(tag: XML_ELEMENT_IDENTIFIER, inNamespace: XML_METADATA_NAMESPACE)?.stringValue
